...
- **当前版本**：见 `src/version.py` 的 `SOFTWARE_VERSION`（当前 `20260910-V2`）。全程序（后端、前端、日志、页面、测试结果）统一使用这一个版本号，不区分前后端版本。
...
- **报文解析**：对 RADIUS 报文进行编解码与字段解析；测试详情展示该用户最新一条认证请求/响应报文及其属性匹配结果（含 VLAN/QoS 等 VSA），便于调试。
