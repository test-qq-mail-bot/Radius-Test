...
- **当前版本**：见 `src/version.py` 的 `SOFTWARE_VERSION`（当前 `20260910-V3`）。全程序（后端、前端、日志、页面、测试结果）统一使用这一个版本号，不区分前后端版本。
...
- **报文解析**：对 RADIUS 报文进行编解码与字段解析；测试详情展示该用户最新一条认证请求/响应报文及其属性匹配结果，
  并按 **隧道/VLAN、QoS 限速、安全组/ACL、会话控制** 分类汇总授权属性（含厂商私有属性 VSA，速率自动换算 Kbps/Mbps）。
