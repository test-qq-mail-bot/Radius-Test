# -*- coding: utf-8 -*-
"""RADIUS 属性中文释义表（授权说明列的数据源）。

说明：
    本模块由 authz.py 拆出，用于满足工程约束「单 .py 文件 ≤20KB」。
    仅承载数据与查询，不依赖 authz，避免循环导入。

内容：
    1. STANDARD_DESCRIPTIONS：标准属性编号 -> 中文释义
       （覆盖 RFC2865 / RFC2866 / RFC2869 / RFC2868 / RFC3162 等）；
    2. VENDOR_DESCRIPTIONS：(厂商编号, 属性编号, 英文名) -> 中文释义
       （覆盖 H3C / HP / Juniper / Mikrotik / Aruba / WISPr 六家内置字典）；
    3. describe_attribute()：对外查询入口，未收录返回空串（前端显示 -）。
"""
...
def describe_attribute(vendor_id, attribute_id, name: str = "") -> str:
    ...
    # authz.VENDOR_ATTRIBUTES 的第 6 元素就是华为属性的中文释义。
    # 该属性定义表仍留在 authz.py：若搬来本模块会突破「单 .py ≤20KB」上限，
    # 因此此处延迟导入，既避免 authz <-> describe 循环依赖，也不重复维护数据。
    from .authz import VENDOR_ATTRIBUTES
